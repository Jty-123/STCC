...
import torchvision
import argparse
from modules import transform, network, contrastive_loss, stcc
...
def train():
    ...
    return loss_epoch, loss_instance, loss_cluster
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.epochs):
        tags = ["train_loss", "instance_loss", "cluster_loss","learning_rate"]
        lr = optimizer.param_groups[0]["lr"]
        loss_epoch, loss_instance, loss_cluster= train()
        if epoch % 100 == 0:
            save_model(args, model, optimizer, epoch)
        print(f"Epoch [{epoch}/{args.epochs}]\t Loss: {loss_epoch / len(data_loader)}")
        train_writer.add_scalar(tags[0],loss_epoch / len(data_loader),epoch)
        train_writer.add_scalar(tags[1],loss_instance / len(data_loader),epoch)
        train_writer.add_scalar(tags[2],loss_cluster / len(data_loader),epoch)
        train_writer.add_scalar(tags[3],optimizer.param_groups[0]["lr"] ,epoch)
    train_writer.close()
